...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            cout << "priority: " << execfile_priority[execfileNum] << "\n";
...
Kernel::Initialize()
{
    ...
    currentThread = new Thread("main", threadNum++);	// (12/17)
...
void ForkExecute(Thread *t)
{
    ...
    t->space->Execute(t->getName()); // allocate space for new thread
...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i], execfile_priority[i]);
	}
